/*
modified from flink-json DebeziumJsonDeserializationSchema.java
 */
 ...
 */
@Internal
public final class CustomDebeziumJsonDeserializationSchema implements DeserializationSchema<RowData> {
    ...
    public CustomDebeziumJsonDeserializationSchema(
    ...
    public void deserialize(byte[] message, Collector<RowData> out) throws IOException {
        ...
        try {
            ...
            if (schemaInclude) {
                ...
                System.out.printf("payload include schema: %s\n", payload);
            } else {
                payload = row;
            ...
            // TimestampData ts = (TimestampData) ReadableMetadata.SOURCE_TIMESTAMP.converter.convert(payload, 3);
            // System.out.printf("before: %s, after: %s, op: %s, ts: %s\n", before, after, op, ts);
            if (OP_CREATE.equals(op)) {
                // after = buildCustomRowData(after, op);
                ...
                // before = buildCustomRowData(before, op);
                emitRow(row, before, out);
            } else {
                if (OP_READ.equals(op)){
                    // do nothing
                    System.out.printf("skip OP_READ: %s\n", row);
    ...
    public boolean equals(Object o) {
        ...
        CustomDebeziumJsonDeserializationSchema that = (CustomDebeziumJsonDeserializationSchema) o;
...
//                        DataTypes.FIELD("op", DataTypes.STRING())
    ...
    private static GenericRowData buildCustomRowData(GenericRowData input, String op) {
        GenericRowData result = new GenericRowData(input.getArity() + 1);
        for (int i = 0; i < input.getArity(); i++) {
            result.setField(i, input.getField(i));
        }
        result.setField(input.getArity(), StringData.fromString(op));
